...
namespace Michsky.UI.ModernUIPack
{
    ...
    public class CustomDropdown : MonoBehaviour, IPointerExitHandler, IPointerEnterHandler, IPointerClickHandler
    {
        ...
        Sprite imageHelper;
        string textHelper;
        private Action languageChangedHandler;
        ...
        void Start()
        {
            ...
            // 🔁 Til o‘zgarganda selectedText yangilansin
            languageChangedHandler = () => UpdateDropdownTexts();
            LanguageManager.OnLanguageChangedEvent += languageChangedHandler;

            UpdateItemLayout();

        }
        public void UpdateDropdownTexts()
        {
            // selectedText ni update qilamiz
            if (selectedItemIndex >= 0 && selectedItemIndex < dropdownItems.Count)
                ChangeDropdownInfo(selectedItemIndex);

            // Pastga tushadigan ro‘yxatdagi itemlarni yangilaymiz
            for (int i = 0; i < itemParent.childCount && i < dropdownItems.Count; i++)
            {
                Transform item = itemParent.GetChild(i);
                TextMeshProUGUI itemText = item.GetComponentInChildren<TextMeshProUGUI>();
                if (itemText != null)
                {
                    int textId = dropdownItems[i].textId;
                    itemText.text = textId >= 0
                        ? LanguageManager.Instance.GetText(textId)
                        : dropdownItems[i].itemName;
                }
            }
        }
        ...
        public void SetupDropdown()
        {
            ...
            for (int i = 0; i < dropdownItems.Count; ++i)
            {
                ...
                textHelper = dropdownItems[i].textId >= 0
                    ? LanguageManager.Instance.GetText(dropdownItems[i].textId)
                    : dropdownItems[i].itemName;
            ...
            try
            {
                selectedText.text = LanguageManager.Instance.GetText(dropdownItems[selectedItemIndex].textId);
        ...
        public void ChangeDropdownInfo(int itemIndex)
        {
            ...
            if (selectedText != null)
                selectedText.text = LanguageManager.Instance.GetText(dropdownItems[itemIndex].textId);
