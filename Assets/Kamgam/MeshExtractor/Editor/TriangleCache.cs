...
namespace Kamgam.MeshExtractor
{
    ...
    public static class TriangleCache
    {
        ...
        /// <summary>
        /// Selects all triangles that have the same material assigned to them as the 'selectdTri'.
        /// </summary>
        /// <param name="objects">The object to search (they need to be cached).</param>
        /// <param name="selectedTri">The triangle to start the link search from.</param>
        /// <param name="maxDuration">Max duration in seconds.</param>
        /// <returns></returns>
        public static List<SelectedTriangle> AddSameMaterial(IList<GameObject> objects, SelectedTriangle selectedTri)
        {
            var results = new List<SelectedTriangle>() { };
            var components = new List<Component>();

            // Get the material of the currently selected triangle.
            Material material = null;
            var materials = getMaterials(selectedTri.Component);
            if (materials != null)
            {
                material = materials[selectedTri.SubMeshIndex];
            }

            if (material == null)
            {
                Logger.LogWarning("The last selected triangle has no material assigned. Will use the submesh index instead.");
            }

            var verticesPerComp = new Dictionary<Component, Vector3[]>();
            var trisPerCompPerSubMesh = new Dictionary<Component, List<int[]>>();
            var materialsPerComp = new Dictionary<Component, Material[]>();
            foreach (var obj in objects)
            {
                if (obj == null)
                    continue;
            ...
            foreach (var comp in components)
            {
                ...
                if (!trisPerCompPerSubMesh.ContainsKey(comp))
                {
                    ...
                    // materials
                    var compMaterials = getMaterials(comp);
                    materialsPerComp.Add(comp, compMaterials);
                }
            }

            EditorUtility.DisplayProgressBar("Working", "Gathering same material tris ..", 0.1f);
            try
            {
                foreach (var comp in components)
                {
                    // Skip other components if the material was null because then we can only use the sub
                    // mesh index and that is local to the component/mesh.
                    if (material == null && comp != selectedTri.Component)
                        continue;

                    var cVertices = verticesPerComp[comp];
                    var cTris = trisPerCompPerSubMesh[comp];
                    var cMaterials = materialsPerComp[comp];

                    {
                        for (int subMeshIndex = 0; subMeshIndex < cTris.Count; subMeshIndex++)
                        {
                            int triCount;
                            // For each submesh
                            if (material == null && comp == selectedTri.Component)
                            {
                                // Copy all tris from sub mesh if material of selected tri is null.
                                if (subMeshIndex == selectedTri.SubMeshIndex)
                                {
                                    triCount = cTris[subMeshIndex].Length;
                                    for (int i = 0; i < triCount; i += 3)
                                    {
                                        var idx0 = cTris[subMeshIndex][i];
                                        var idx1 = cTris[subMeshIndex][i + 1];
                                        var idx2 = cTris[subMeshIndex][i + 2];
                                        var v0 = cVertices[idx0];
                                        var v1 = cVertices[idx1];
                                        var v2 = cVertices[idx2];
                            ...
                            {
                                // Ignore sub meshes that have no material assigned (they would never mach).
                                // For all other sub meshes check if they have the same material as the selected triangle.
                                // If yes, then add them.
                                if (cMaterials.Length > subMeshIndex && cMaterials[subMeshIndex] == material)
                                {
                                    // Copy all tris that have the same material as the selected tri.
                                    triCount = cTris[subMeshIndex].Length;
                                    for (int i = 0; i < triCount; i += 3)
                                    {
                                        var idx0 = cTris[subMeshIndex][i];
        ...
        private static Material[] getMaterials(Component comp)
        {
            var meshFilter = comp as MeshFilter;
            if (meshFilter != null)
            {
                return meshFilter.transform.GetComponent<MeshRenderer>().sharedMaterials;
            }
            var skinnedRenderer = comp as SkinnedMeshRenderer;
            if (skinnedRenderer != null)
            {
                return skinnedRenderer.sharedMaterials;
            }
