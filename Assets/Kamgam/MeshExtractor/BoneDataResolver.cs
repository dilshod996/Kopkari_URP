    ...
    public class BoneDataResolver : MonoBehaviour
    {
        ...
        public bool Resolve()
        {
            if (BoneData != null && Renderer != null)
            {
                ...
                // If the existing root bone already matches the root bone name then use that.
                if (existingRootBone.gameObject.activeSelf && existingRootBone.name == BoneData.RootName)
                    return Resolve(existingRootBone);
