...
public class GalleryLevelSelectionManager : MonoBehaviour
{
    ...
    public virtual void Start()
    {
        if (Application.isPlaying)
        {
            ...
            isOverlayCanvas = canvas && canvas.renderMode == RenderMode.ScreenSpaceOverlay;
        }

        // Initialize pointer input actions (works for both mouse & touch)
        pointerAction = new InputAction(type: InputActionType.PassThrough, binding: "<Pointer>/position");
        pointerAction.Enable();

        // Handle Pointer Press (Click or Touch Start)
        InputAction pointerPressAction = new InputAction(type: InputActionType.PassThrough, binding: "<Pointer>/press");
        pointerPressAction.started += ctx => OnMouseButtonDown();  // Press start
        pointerPressAction.performed += ctx => OnMouseButtonMove(); // Dragging
        pointerPressAction.canceled += ctx => OnMouseButtonUp();    // Release
        pointerPressAction.Enable();

        // ✅ Enable Enhanced Touch Support for multi-touch (only if touchscreen exists)
        if (Touchscreen.current != null)
        {
            EnhancedTouchSupport.Enable();
            TouchSimulation.Enable(); // Allows testing touch input with a mouse in the editor

            // Listen for actual finger touches
            UnityEngine.InputSystem.EnhancedTouch.Touch.onFingerDown += ctx => OnMouseButtonDown();
            UnityEngine.InputSystem.EnhancedTouch.Touch.onFingerMove += ctx => OnMouseButtonMove();
            UnityEngine.InputSystem.EnhancedTouch.Touch.onFingerUp += ctx => OnMouseButtonUp();
        }
    }
    ...
    public virtual void OnItemsClicked(int index, bool isCenter)
    {
        ...
        // Message.Send<Widebrain.Data.CharacterSelectURL>(new Widebrain.Data.CharacterSelectURL { index = index.ToString() });
    ...
    public virtual void ShowItem(int number, bool animate)
    {
        ...
        int temp = Mathf.Clamp(number, 0, items.Length-1);
    ...
    public void AutoGenerateItems()
    {
        ...
        for (int i = 0; i < autoGenerateItemsCount; i++)
        {
            ...
            if (autoGenerateAddText)
            {
                ...
                TMP_Text textComp = txt.AddComponent<TMP_Text>();
    ...
    {
        Vector2 touchPosScreen = pointerAction.ReadValue<Vector2>();
        // UI Interaction Check (Touch or Mouse)
        PointerEventData eventData = new PointerEventData(EventSystem.current)
        {
            position = touchPosScreen
        };
        ...
        {
            touchPosScreen = Touchscreen.current.primaryTouch.position.ReadValue();
        }

        itemClickedDown = null;
        clickedLevelIndex = -1;

        Collider2D[] hit = Physics2D.OverlapPointAll(touchPosScreen);
        ...
        foreach (Collider2D h in hit)
        {
            if (h == galleryCollider)
            {
                ...
                touchPosOnMouseDown = touchPosScreen;  // ✅ FIXED: No more `Input.mousePosition`
                touchPosWorldOnMouseDown = touchPosScreen;
        ...
        // 🔹 Final UI Slider Detection Fix
        if ((indicatorType == IndicatorType.Slider || indicatorType == IndicatorType.Scrollbar) &&
            sliderCollider == Physics2D.OverlapPoint(touchPosScreen))
        {
            isTouchedOnSlider = true;
        }
    }
    private void OnMouseButtonMove()
    {
        ...
        {
            Vector2 touchPos = pointerAction.ReadValue<Vector2>(); // Get pointer position

            if (Vector2.Distance(touchPos, touchPosWorldOnMouseDown) >= moveThreshold)
            {
                float xPos = galleryChildPosOnMouseDown.x + (touchPos.x - touchPosOnMouseDown.x) * speedMultiplier;
                if (scrollAxis == ScrollAxis.Vertical)
                    xPos = galleryChildPosOnMouseDown.y + (touchPos.y - touchPosOnMouseDown.y) * speedMultiplier;
                ...
                // Silliq harakat qilish uchun Lerp
                galleryChild.transform.localPosition = Vector2.Lerp(galleryChild.transform.localPosition, pos, Time.deltaTime * 20f);
    ...
    //    {
    //        Vector2 touchPos = pointerAction.ReadValue<Vector2>(); // Get pointer position
    //        //Vector2 touchPosWorld = isOverlayCanvas ? touchPos : Camera.main.ScreenToWorldPoint(new Vector3(touchPos.x, touchPos.y, Camera.main.nearClipPlane));

    //        if (Vector2.Distance(touchPos, touchPosWorldOnMouseDown) >= moveThreshold)
    //        {
    //            float xPos = galleryChildPosOnMouseDown.x + (touchPos.x - touchPosOnMouseDown.x);
    //            if (scrollAxis == ScrollAxis.Vertical)
    //                xPos = galleryChildPosOnMouseDown.y + (touchPos.y - touchPosOnMouseDown.y);
    ...
    //private void OnMouseButtonMove()
    //{
    //    Debug.Log("OnMouseButtonMove");

    //    if (isScrollable && isTouchOnGallery && !isTouchedOnSlider)
    //    {
    //        Vector2 touchPos = pointerAction.ReadValue<Vector2>(); // Get pointer position
    //                                                               // No need to convert to world space, since the canvas is Screen Space - Overlay
    //        Vector2 touchPosScreen = touchPos;

    //        // Check if the movement exceeds the threshold
    //        if (Vector2.Distance(touchPosScreen, touchPosOnMouseDown) >= moveThreshold)
    //        {
    //            // Calculate movement delta
    //            float deltaX = touchPos.x - touchPosOnMouseDown.x;

    //            // Convert movement into a scroll value
    //            float newScrollPos = horizontalNormalizedPosition - (deltaX / (Screen.width * dragSpeedFactor));

    //            // Clamp the scroll value within bounds
    //            newScrollPos = Mathf.Clamp(newScrollPos, 0, 1);

    //            // ✅ Update both the gallery position and scrollbar value
    //            setHorizontalNormalizedPosition(newScrollPos);
    //            UpdateGalleryStateOnIndicator();

    //            // Save the current touch position for smooth dragging
    //            touchPosOnMouseDown = touchPos;
    //        }
    //    }
    //}
    ...
    {
        Vector2 touchPosWorld = GetPointerPosition();
    ...
    private void OnGalleryPositionChange()
    {
        ...
        if (infiniteLoop)
        {
            if ((int)(scroll / scrollStep) + 1 > items.Length)
            {
                ...
                // ✅ FIXED: Replace Input.mousePosition
                touchPosOnMouseDown = pointerAction.ReadValue<Vector2>();
                scroll = 0;
            }
        ...
        float totalRange = items.Length > 1 ? (mActiveItems / 2) * scrollStep : 1;
        ...
        for (int i = start; i <= end; i++)
        {
            ...
            scale = linearScale
                ? (linearProgress < 1 ? 1 - ((1 - minScaleDownValue) * linearProgress) : minScaleDownValue)
                : minScaleDownValue + ((1 - progress) * (1 - minScaleDownValue));
            ...
            if (levelTitleFormat != LevelTitleFormat.None)
            {
                color.a = Mathf.Abs(scroll - (i * scrollStep)) < scrollStep / 3 ? 1 - (Mathf.Abs(scroll - (i * scrollStep)) / scrollStep) * 3 : (showTextNumber ? color.a : 0);
                showTextNumber = true;
            }
        }

        if (items.Length == 1) color.a = 1;
        ...
        if (!infiniteLoop || start >= 0)
        {
            if (start > 0)
            {
                firstItemPos = ((start - 1) * (((mItemsWidth) * minScaleDownValue) + spaceBetweenItems)) +
                               (((mItemsWidth / 2) * minScaleDownValue) + spaceBetweenItems) + ((mItemsWidth / 2) * itemsScale[0]);
            }
        ...
        {
            int tempI = GetInfiniteLoopIndex(i);
            itemsPos[i - start].y = displacementType == DisplacementType.Circular
                ? GetYPosOfCircularDisplacement(itemsPos[i - start].x + centerXPos)
                : (displacementType == DisplacementType.Linear ? itemsLinearProgress[i - start] : itemsProgress[i - start]) * maxDisplacementPos;
    ...
    private int GetInfiniteLoopIndex(int index)
    {
        ...
        {
            if (index < 0)
                temp = items.Length + index; // Manfiy index uchun to'g'ri hisoblash
            else if (index >= items.Length)
                temp = index - items.Length; // Kengaytirilgan indexlar uchun to'g'ri hisoblash
        }
        return temp;
    }
    ...
    private void setHorizontalNormalizedPosition(float value)
    {
        ...
        if (items.Length <= 1) pos.x = 0; // Bitta elementda x o'zgarmas
        SwapIfVertical(ref pos);
        galleryChild.transform.localPosition = pos;
    }
